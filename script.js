...
/* FORM */
const specialtySelect = document.getElementById('rcmSpecialty');
const otherSpecialtyGroup = document.getElementById('otherSpecialtyGroup');
const otherSpecialtyInput = document.getElementById('rcmOtherSpecialty');
const referralSourceSelect = document.getElementById('rcmReferralSource');
const otherReferralSourceGroup = document.getElementById('otherReferralSourceGroup');
const otherReferralSourceInput = document.getElementById('rcmOtherReferralSource');

function toggleOtherSpecialty() {
  const isOther = specialtySelect.value === 'Other';
  otherSpecialtyGroup.style.display = isOther ? 'block' : 'none';
  otherSpecialtyInput.required = isOther;
  if (!isOther) otherSpecialtyInput.value = '';
}

specialtySelect.addEventListener('change', toggleOtherSpecialty);
toggleOtherSpecialty();

function toggleOtherReferralSource() {
  const isOther = referralSourceSelect.value === 'Other';
  otherReferralSourceGroup.style.display = isOther ? 'block' : 'none';
  otherReferralSourceInput.required = isOther;
  if (!isOther) otherReferralSourceInput.value = '';
}

referralSourceSelect.addEventListener('change', toggleOtherReferralSource);
...
async function submitForm() {
  ...
  const specialty = specialtySelect.value, otherSpecialty = otherSpecialtyInput.value.trim(), providers = document.getElementById('rcmProviders').value, collections = document.getElementById('rcmCollections').value, referralSource = referralSourceSelect.value, otherReferralSource = otherReferralSourceInput.value.trim(), challenge = f('rcmChallenge');
  if (!firstName || !lastName || !email || !phone || !practice) { setStatus('error', 'Please fill in all required fields marked with *'); return; }
  if (specialty === 'Other' && !otherSpecialty) { setStatus('error', 'Please specify your specialty.'); return; }
  if (referralSource === 'Other' && !otherReferralSource) { setStatus('error', 'Please specify how you heard about us.'); return; }
  ...
  try {
    await fetch(GAS_URL, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ firstName, lastName, email, phone, practice, specialty: specialty || 'Not specified', otherSpecialty: specialty === 'Other' ? otherSpecialty : 'Not specified', providers: providers || 'Not specified', collections: collections || 'Not specified', referralSource: referralSource || 'Not specified', otherReferralSource: referralSource === 'Other' ? otherReferralSource : 'Not specified', challenge: challenge || 'Not provided' }) });
...
const phoneInput = document.getElementById('rcmPhone');

phoneInput.addEventListener('input', (e) => {
  let value = e.target.value.replace(/\D/g, ''); // Remove all non-digits
  if (value.startsWith('1')) value = value.substring(1); // Handle the +1 separately
